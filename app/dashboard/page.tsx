...
import Feedback from "@/components/orgamisms/Feedback";
...
const page = () => {
  ...
  const data = useMemo(() => {
    if (moodData.length) {
      return [
        ["", "Mood"],
        ["😍", moodData.filter((e:IFeedback) => Math.ceil(e.rating as number) === 5).length],
        ["😊", moodData.filter((e:IFeedback) => Math.ceil(e.rating as number) === 4).length],
        ["🙂", moodData.filter((e:IFeedback) => Math.ceil(e.rating as number) === 3).length],
        ["😟", moodData.filter((e:IFeedback) => Math.ceil(e.rating as number) === 2).length],
        ["😡", moodData.filter((e:IFeedback) => Math.ceil(e.rating as number) === 1).length],
      ];
    } else {
      return [
        ["", "Mood"],
  ...
  const getFeedbackData = async (date: any) => {
    ...
    const mood  = feedbackData.data.filter((e:IFeedback)=>e.type==='dailyMood')[0].data
    setMoodData(mood)
  };

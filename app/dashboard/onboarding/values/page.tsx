...
const OnboardingValues = () => {
  ...
  const getBannerData = async () => {
    ...
    const data: ApiResponse<any> = await fetchData(
      `api/onboarding/values/?email=${email}`,
      {
        method: "GET",
      }
    );
              ...
              {valuesData.map((item: any, index) => {
                ...
                return (
                  <div key={index} className="border-b-2 border-black mb-4">
                    <p className="font-bold text-xl">{item.days}</p>
                    {isObject &&
                      keys.map((ans: any, i: number) => {
                        ...
                        return (
                          <div key={i} className="px-10 py-4 space-y-1">
                            ...
                            {question.map((e, key) => (
                              <div className="flex justify-between px-24 gap-4">
                                <p>{e}</p>
                                <p>{answer[key]}</p>
                              </div>
                            ))}
...
export default OnboardingValues;
